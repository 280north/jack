...
exports.Directory = function(root, app) {
    ...
    return function(env) {
        try {
            ...
            var path = file.join(root, pathInfo, scriptName);
            ...
            if (file.isReadable(path)) {
                if (file.isFile(path)) {
                    ...
                    var body = generateListing(root, pathInfo).toByteString("UTF-8");
                    return [403, { "Content-Type" : "text/html; charset=utf-8", "Content-Length" : String(body.length) }, [body]];
                }
            }
        } catch (e) {
        }
        
        var body = ("Entity not found: "+pathInfo+"\n").toByteString("UTF-8");
...
function byteSizeFormat(size) {
    var tier = Math.floor(Math.log(size) / Math.log(1024));
