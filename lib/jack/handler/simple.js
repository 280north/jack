...
exports.run = function(app, options) {
    ...
    var port = options["port"] || 8080,
        commandPort = options["command_port"] || (port + 1),
        ...
        commandAddress = new Packages.java.net.InetSocketAddress(commandPort),
    ...
    var commandHandler = new JavaAdapter(Packages.org.simpleframework.http.core.Container, {
        handle : function(request, response) {
            try {
    ...
    if (typeof Packages.org.simpleframework.transport.connect.SocketConnection === "function") {
        ...
        commandConnection = new Packages.org.simpleframework.transport.connect.SocketConnection(commandHandler);
    }
        ...
        commandConnection = new Packages.org.simpleframework.http.connect.SocketConnection(commandHandler);
    }
    else {
        throw new Error("Simple SocketConnection not found, missing .jar?");
    }
    
    print("Jack is starting up using Simple on port " + port + " with command port " + commandPort);
    
    connection.connect(address);
    commandConnection.connect(commandAddress);
}
