...
Servlet.process = function(options, request, response) {
    ...
    if(!processor){
        var global = workerEngine.createEnvironment();
            // add the module lookup paths from our environment
            var paths = global.require.paths;
            paths.splice(0, paths.length);
            paths.push.apply(paths, require.paths);

        options.server = "servlet";
        var processor = global.require("jackup").start(options);
...
exports.run = function(app, options){
    ...
    return sync(function(request, response){
        ...
        req.scriptName         = String(request.getServletPath() || "");
