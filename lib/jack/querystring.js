...
QSUtils.unescape = function (str, decodeSpaces) {
    // unclear whether plusses are reserved in the URI path
    if (decodeSpaces) str = str.replace(/\+/g, " ");
    return decodeURIComponent(str);
};

QSUtils.escape = function (str) {
	    ...
	    return function parsePiece (key, val) {
    		if (arguments.length !== 2) {
    			// key=val, called from the map/reduce
    		 	key = key.split(eq);
    			return parsePiece(
    			    QSUtils.unescape(key.shift(), true), QSUtils.unescape(key.join(eq), true)
    			);
    		}
