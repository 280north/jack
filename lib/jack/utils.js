...
Utils.parseQuery = function(queryString) {
    ...
    for (var i = 0; i < pieces.length; i++) {
        var kv = Utils.unescape(pieces[i]).split("=", 2), key = kv[0], value = kv[1];
        if (key) {
            var cur = params[key];
            if (cur) {
...
Utils.parseMultipart = function(env) {
    ...
    if (env['CONTENT_TYPE'] && (match = env['CONTENT_TYPE'].match(/^multipart\/form-data.*boundary=\"?([^\";,]+)\"?/m))) {
        ...
        var status = input.read(boundaryLength).toString();
        ...
        while (true) {
            var head = null,
                body = {
                    // TODO: make this a proper string buffer / stream
                    _buffer : [],
                    write : function(string) { this._buffer.push(string); },
            ...
            while (!head || !rx.test(buf)) {
                if (!head && (i = buf.indexOf("\r\n\r\n")) > 0) {
                    ...
                    if (filename) {
                        system.log.warn("multipart file not implemented");
                        // TODO: bypass String conversion for files, use Binary/ByteString directly
                        // FIXME: use a temporary file
                        // huge vulnerability! don't let user control location of uploaded file
                        //body = File.open(filename, "w");
                    }
            ...
            if (filename) {
                ...
                data = {
                    ...
                    "type"      : contentType,
                    "name"      : name,
                    "tempfile"  : filename,//File.open(filename, "r"),
                    "head"      : head
                };
            } else {
                data = body.toString();
