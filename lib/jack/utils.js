...
Utils.parseQuery = function(queryString, d) {
    var params = {},
        pieces = String(queryString || "").split(d ? new RegExp("["+d+"] *", "g") : DEFAULT_SEP);
    for (var i = 0; i < pieces.length; i++) {
        var kv = pieces[i].split("=").map(Utils.unescape), key = kv[0], value = kv[1];
        if (key) {
            var cur = params[key];
            if (cur) {
