...
exports.start = function(options) {
    ...
    if (!options.code) {
        ...
        if (config.charAt(0) !== "/" && !config.match(/^\w:[\\\/]/))
        ...
        if (options.reload) {
            print("Module reloading is turned on for development. Ensure that it is turned off for deployed applications for better performance");
    ...
    {
        app = configModule[options.environment](app, options);
    }
    else
    {
        switch (options.environment) {
            case "development" :
                app = require("jack/commonlogger").CommonLogger(
                        require("jack/showexceptions").ShowExceptions(app));
                break;
            case "deployment" :
